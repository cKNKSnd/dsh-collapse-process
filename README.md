...
- **默认折叠**：隐藏对话中的“过程”内容 —— 思考过程（Think 披露）、工具调用卡片、steering、命令、上下文注入、压缩、模型重试、回合错误/截断、工作流运行等
- **一键切换**：会话标题栏右侧出现“显示过程 / 隐藏过程”按钮，点击切换
- **偏好记忆**：折叠/展开选择保存在浏览器本地（localStorage），刷新或重开会话后保持
- **无闪烁启动**：bundle 随页面立即加载（`dsh.client.immediately`），样式在会话渲染前生效
- 用户消息与最终回答**不受影响**
- 界面文案跟随 DSH 语言（中文 / English），按钮带键盘焦点环（focus-visible）
