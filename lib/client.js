window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * 服务依赖（fiber 激活门控）：slots 注册按钮；locale 提供词典与 t 座。
     * 必须声明 locale，否则 bundle 在 immediately 模式下会先于 locale
     * 服务激活，词典注册被跳过、按钮文案退化为原始 key。
     */
    ...
    function apply(ctx) {
      ...
      // 多语言：绑定稳定翻译函数（bind 与插槽注入的 t 座同源），
      // 再注册词典；同 namespace 残留注册（词典相同）时幂等容忍。
      const locale = ctx.get('locale');
      const boundT = locale !== undefined ? locale.bind(NS) : null;
      let disposeLocale = null;
      if (locale !== undefined) {
        try {
          disposeLocale = locale.register(NS, { zh, en });
        } catch (err) {
          console.warn('[dsh-collapse-process] locale namespace already registered:', err);
          disposeLocale = null;
        }
      }
      ctx.effect(() => () => {
      ...
      function ToggleButton(props) {
        ...
        // 优先用 apply 内绑定的 t（保证与已注册词典一致），
        // 其次用插槽注入的 t 座，最后回退到中文词典。
        const t = boundT || ((props && props.t) || ((key) => zh[key] || key));
        react.useEffect(() => {
          applyCss(isHidden);
        }, [isHidden]);
